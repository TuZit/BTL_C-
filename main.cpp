...
public:
    ...
    User() : points(0), role(USER), isAutoGenerated(false) {}
     ...
     */
    User(string uname, string pwd, string fname, Role userRole = USER, bool autoGen = false, int pts = 10 )
    {
        username = uname;
        password = pwd;
        fullName = fname;
...
vector<User> loadUsers()
{
    ...
    {
        User user;
        user.username = userJson["username"].get<string>();
        user.password = userJson["password"].get<string>();
        user.fullName = userJson["fullName"].get<string>();
        user.role = userJson["role"].get<string>() == "ADMIN" ? ADMIN : USER;
        user.isAutoGenerated = userJson["isAutoGenerated"].get<bool>();
        user.points = userJson["points"].get<int>();
        user.userID = userJson["userID"].get<string>();

        // Xóa ví và giao dịch cũ (phòng trường hợp load lại nhiều lần)
        user.wallets.clear();
...
User *login(vector<User> &users)
{
    ...
    for (auto &user : users)
    {
        string hashedInput = user.isAutoGenerated ? password : user.hashPassword(password);
